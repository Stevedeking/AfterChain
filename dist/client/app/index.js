...
async function runLivenessMonitor(env2) {
  const rpc = env2.VITE_ETHERLINK_RPC || "https://node.ghostnet.etherlink.com";
  ...
  const REGISTRY = env2.REGISTRY_ADDRESS || "";
  ...
  for (const willAddr of wills) {
    try {
      ...
      if (daysRemaining === 0) {
        console.log(`[Afterchain Cron] ${willAddr} heartbeat expired (last: ${new Date(lastHeartbeat * 1e3).toISOString()}) — triggering executeWill`);
